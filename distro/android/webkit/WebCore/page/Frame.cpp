...
Frame::Frame(Page* page, HTMLFrameOwnerElement* ownerElement, FrameLoaderClient* frameLoaderClient)
    ...
    , m_renderRect(0)
    , m_swathRect(0)
    , m_renderReverseOrder(false)
    , m_renderGrayscale(false)
    , m_renderBackground(false)
    , m_renderPrintContext(0)
    , m_printGraphicsContext(0)
    , m_graphicContext(0)
    , m_bitmapDevice(0)
{
...
int Frame::renderBegin(int pageWidth, int pageHeight, int printDpi, bool renderLandscape)
{
    /* setup render variables */
    m_renderLandscape    = renderLandscape;
    m_renderPrintDpi = printDpi;
    m_renderZoomFactor = printDpi / 96.0;
    m_renderSwathHeight = printDpi;
    if (!m_renderLandscape)
    {
        m_renderPageWidth = pageWidth;
        m_renderPageHeight = pageHeight;
    }
    else
    {
        m_renderPageWidth = pageHeight;
        m_renderPageHeight = pageWidth;
    }
    m_renderPageRect = FloatRect(0, 0, m_renderPageWidth / m_renderZoomFactor, m_renderPageHeight / m_renderZoomFactor);

    /* create the platform graphic context */
    m_printGraphicsContext = new PlatformGraphicsContext();
    if (!m_printGraphicsContext)
        return(0);
    
    /* create the bitmap we will use to render */
    m_bitmapDevice = new SkBitmap();
    if (!m_bitmapDevice)
        return(0);

    m_swathRect  = new IntRect();
    m_renderRect = new IntRect(0,0, pageWidth, pageHeight);
    m_sourceRect = new IntRect(0,0, m_renderPageWidth, m_renderPageHeight);

    /* create the varios rectangles */
    if (!m_sourceRect || !m_renderRect || !m_swathRect)
        return(0);

    m_lastRenderedPageIndex = -1;
    if (m_renderLandscape)
        m_bitmapDevice->setConfig(SkBitmap::kARGB_8888_Config, m_renderSwathHeight, pageWidth);
    else
        m_bitmapDevice->setConfig(SkBitmap::kARGB_8888_Config, pageWidth, m_renderSwathHeight);
    m_bitmapDevice->allocPixels();
    m_bitmapDevice->eraseColor(SK_ColorWHITE);
    m_printGraphicsContext->mCanvas->setBitmapDevice(*m_bitmapDevice);

    /* create the graphic context */
    m_graphicContext = new GraphicsContext(m_printGraphicsContext);
    if (!m_graphicContext)
        return(0);

    /* create the print context */
    m_renderPrintContext = new PrintContext(this);
    if (!m_renderPrintContext)
        return(0);

    /* begin print rendering */
    m_renderPrintContext->begin(m_renderPageRect.width());

    /* compute the number of pages */
    float layoutHeight;
    m_renderPrintContext->computePageRects(m_renderPageRect,
                                           0,                //headerHeight
                                           0,                //footerHeight
                                           1.0,              //userScaleFactor
                                           layoutHeight);
    ...
    return m_renderPrintContext->pageCount();
}

void Frame::renderSetReverseOrder(bool renderReverseOrder)
{
    m_renderReverseOrder = renderReverseOrder;
}

void Frame::renderSetGrayscale(bool renderGrayscale)
{
    m_renderGrayscale = renderGrayscale;
}

void Frame::renderSetBackgroundPrinting(bool renderBackground)
{
    if (renderBackground != m_renderBackground) {
        m_renderBackground = renderBackground;

        if (settings()) {
            settings()->setShouldPrintBackgrounds(m_renderBackground);
        }
    }
}

void Frame::remapRenderBand()
{
    int x, y, width, height;

    if (m_renderLandscape)
    {
        if (m_renderReverseOrder)
        {
            x      = m_renderRect->height() - m_swathRect->bottom();
            y      = m_renderRect->width() - m_swathRect->width();
            width  = m_swathRect->height();
            height = m_swathRect->width();
        }
        else
        {
            x      = m_swathRect->y();
    ...
    else
    {
        ...
        {
            x      = m_renderRect->x();
            y      = m_renderRect->height() - m_swathRect->bottom();
            width  = m_swathRect->width();
            height = m_swathRect->height();
        }
        else
        {
            x      = m_swathRect->x();
    ...
    IntRect bandRect(x,y,width,height);

    /* render the data */
    m_renderPrintContext->spoolPageBand(*m_graphicContext, m_lastRenderedPageIndex - 1, m_renderPageRect.width(), bandRect, m_renderZoomFactor);
}

void Frame::checkRenderSwathHeight(int page, int y, int& swathHeight)
{
    ...
    int pageHeight;
    if (m_renderLandscape || m_renderReverseOrder)
    {
        pageHeight = m_renderRect->height();
    }
    else
    {
        /* get the page height */
        pageHeight = m_sourceRect->height();
    }

    /* determine how many rows we can actually render */
    int h;
    if (y >= pageHeight)
        h = 0;
    else if (swathHeight <= (pageHeight - y))
        h = swathHeight;
    else
        h = pageHeight - y;

    /* adjust the swathHeight variable accordingly */
    swathHeight = h;
}

bool Frame::renderSwath(int page, int y, int swathHeight, int* pixelArray, char *rgbArray, bool reverseRGB, bool mayRender)
{
    /* check that we can actually render rows */
    if (!m_renderPrintContext || (!pixelArray && !rgbArray) || (swathHeight == 0))
        return true;

    /* are we changing pages? */
    if (page != m_lastRenderedPageIndex)
    {
        int pageHeight = m_renderPrintContext->getPageHeight(page - 1, m_renderPageRect, m_renderZoomFactor);
        m_sourceRect->setHeight(pageHeight);
        m_swathRect->setX(-1);
        m_swathRect->setY(-1);
        m_swathRect->setWidth(0);
        m_swathRect->setHeight(0);
        m_lastRenderedPageIndex = page;
    }
    
    int rgba;
    int blue, red, green, alpha;
    int offset, rowsInBuffer;
    int row_increment, col_increment;
    int row_start, col_start;

    while(swathHeight > 0)
    {
        offset = y % m_renderSwathHeight;
        /* check if we have data to copy or if we need to render it */
        if (!((y >= m_swathRect->y()) &&
              (y < m_swathRect->bottom())))
        {  // need to render, can we (are we in the right thread?)
            if (mayRender) {
                int startRow = (y - offset);

                m_swathRect->setX(0);
                m_swathRect->setY(startRow);
                m_swathRect->setWidth(m_renderRect->width());
                m_swathRect->setHeight(min(m_renderSwathHeight, (m_renderRect->height() - startRow)));
        ...
        if (m_renderLandscape)
        {
            if (m_renderReverseOrder)
            {
                 row_start = (m_swathRect->height() - offset - 1);
                 row_increment = -1;
                 col_start = 0;
                 col_increment = 1;
            }
            else
            {
                 row_start = offset;
                 row_increment = 1;
                 col_start = (m_renderRect->width() - 1);
                 col_increment = -1;
            }
        }
        else
        ...
        for(int i = row_start;
            ((i >= 0) && (i < m_renderSwathHeight) && (y < m_renderRect->height()) && (swathHeight != 0));
            i += row_increment, y++, swathHeight--)
        {
            for(int c = col_start; ((c >= 0) && (c < m_renderRect->width())); c += col_increment)
            {
                if (m_renderLandscape)
                    rgba = *(m_bitmapDevice->getAddr32(i, c));
                else
                    rgba = *(m_bitmapDevice->getAddr32(c, i));
                blue  = ((rgba >> 16) & 0xff);                 // convert ABGR => ARGB
                red   = ((rgba >>  0) & 0xff);
                green = ((rgba >>  8) & 0xff);
                alpha = ((rgba >> 24) & 0xff);

                if (m_renderGrayscale)
                {
                    int gray = (((red << 6) + (green * 160) + (blue << 5)) >> 8);
                    red = gray;
                    green = gray;
                    blue = gray;
                }
                ...
                {
                    // convert ABGR => ARGB
                    *pixelArray = ((alpha << 24) | (red << 16) | (green << 8) | (blue << 0));
                     pixelArray++;
                }
                else if (rgbArray)
                {
                    rgbArray[1] = (char)green;
                    if (reverseRGB)
                    {
                        rgbArray[2] = (char)red;
                        rgbArray[0] = (char)blue;
                    }
                    else
                    {
                        rgbArray[0] = (char)red;
                        rgbArray[2] = (char)blue;
                    }
                    rgbArray += 3;
                }
            }
...
void Frame::renderEnd()
{
    if (m_renderPrintContext)
    {
        m_renderPrintContext->end();
        delete m_renderPrintContext;
        m_renderPrintContext = 0;
    }
    ...
    else if (m_printGraphicsContext)
    {
        delete m_printGraphicsContext; /* deleted by GraphicContext */
        m_printGraphicsContext = 0;
    }
